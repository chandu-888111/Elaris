...
export function matchKey(text: string): string | null {
  ...
  if (
    ...
    t.includes("cloud fundamentals") ||
    t.includes("cloud architecture")
  )
    return "cloud";
    ...
    react: [
      {
        ...
        answer:
          "React builds an in-memory cache of the UI structure (the Virtual DOM). When state updates, React renders a new virtual tree, computes the diff (reconciliation) against the real DOM, and updates only the changed nodes for high performance.",
      },
      {
        question: "What are React Hooks and what rules govern them?",
        answer:
          "Hooks are functions that let you hook into React state and lifecycle features from functional components. Rules: 1. Only call Hooks at the top level (not inside loops or conditions). 2. Only call Hooks from React Function Components or Custom Hooks.",
      },
      {
        question: "What is the difference between props and state?",
        answer:
          "Props are read-only inputs passed into a component by its parent, used to configure it. State is a private, mutable data store managed internally by the component itself to track interactive changes.",
      },
    ],
    nextjs: [
      {
        ...
        answer:
          "Server Components are rendered on the server, meaning they send zero JavaScript to the client, improving load speed. Client Components are hydrated on the client, enabling interactivity (like useState, event listeners, and browser APIs). Use 'use client' to declare them.",
      },
      {
        question: "What are SSR, SSG, and ISR in Next.js?",
        answer:
          "SSR (Server-Side Rendering) fetches data and renders pages on every request. SSG (Static Site Generation) compiles pages once at build time. ISR (Incremental Static Regeneration) rebuilds static pages in the background after a specified interval when requests come in.",
      },
    ],
    javascript: [
      {
        ...
        answer:
          "Event delegation is a technique where you attach a single event listener to a parent element rather than multiple listeners to individual child elements. It relies on event bubbling, where the event propagates up through child nodes to the parent.",
      },
      {
        question: "What is a closure in JavaScript?",
        answer:
          "A closure is the combination of a function bundled together with references to its surrounding state (the lexical environment). Closures allow an inner function to access variables from an outer function scope even after the outer function has completed execution.",
      },
    ],
    typescript: [
      {
        ...
        answer:
          "Interfaces are extendable via declaration merging (you can declare the same interface twice and they merge) and are preferred for objects. Type Aliases can define primitives, unions, and tuples, but cannot be reopened to add new properties.",
      },
      {
        question: "What are generics in TypeScript?",
        answer:
          "Generics allow you to build reusable components or functions that work over a variety of types rather than a single one, preserving type safety (e.g. `Array<T>` or `Promise<T>`).",
      },
    ],
    python: [
      {
        ...
        answer:
          "Python manages memory via a private heap space that holds all Python objects. The Python interpreter allocates heap space automatically and recovers unused space via a built-in garbage collector that counts references and handles cyclic references.",
      },
      {
        question: "What are list comprehensions and generator expressions?",
        answer:
          "List comprehensions offer a concise syntax to create lists (e.g. `[x*2 for x in items]`). Generator expressions use parentheses and yield values one at a time lazily (e.g. `(x*2 for x in items)`), saving memory on large streams.",
      },
    ],
    docker: [
      {
        ...
        answer:
          "A Docker Image is a read-only, static template containing instructions for creating a container (like a blueprint). A Docker Container is a running, writeable instance of an image running in isolation.",
      },
      {
        question: "Explain the purpose of Docker Compose.",
        answer:
          "Docker Compose is a tool for defining and running multi-container Docker applications. You declare your services, networks, and volumes in a single `docker-compose.yml` file, and deploy the entire stack using `docker-compose up`.",
      },
    ],
    kubernetes: [
      {
        ...
        answer:
          "A Pod is the smallest deployable unit in Kubernetes. It represents a single instance of a running process and can contain one or more tightly coupled containers sharing storage and network IP addresses.",
      },
      {
        question: "What does a Kubernetes Service do?",
        answer:
          "A Service is an abstraction that defines a logical set of Pods and a policy to access them. It provides a stable network IP address and DNS name, acting as a load balancer to route traffic across dynamic Pods.",
      },
    ],
    aws: [
      {
        ...
        answer:
          "An IAM User is a permanent identity associated with a specific person or application with credentials (password/access keys). An IAM Role is an identity with temporary permissions that can be assumed by any trusted entity (like an EC2 instance or Lambda).",
      },
      {
        question: "What is AWS Lambda?",
        answer:
          "AWS Lambda is a serverless compute service that runs your code in response to events (e.g., S3 upload, API request) and automatically manages the underlying compute infrastructure, billing you only for active execution time.",
      },
    ],
    dsa: [
      {
        ...
        answer:
          "A Hash Map offers O(1) average time complexity for searches, but can degrade to O(N) in case of high hash collisions. A Balanced Binary Search Tree offers O(log N) search complexity in both average and worst cases.",
      },
      {
        question:
          "Explain the difference between Depth First Search (DFS) and Breadth First Search (BFS) in graphs.",
        answer:
          "DFS explores as deep as possible along each branch before backtracking, utilizing a Stack (or recursion). BFS explores all neighbor nodes at the current depth before moving to nodes at the next level, utilizing a Queue.",
      },
    ],
    cybersecurity: [
      {
        ...
        answer:
          "XSS is a vulnerability where malicious scripts are injected into trusted websites and executed in user browsers. Prevention: Sanitizing and encoding all user input before rendering it in the DOM, and setting robust Content Security Policies (CSP).",
      },
      {
        question: "What is the purpose of a Salt in cryptography and password hashing?",
        answer:
          "A Salt is a random string of data appended to a password before hashing. It ensures that identical passwords yield completely different hashes, protecting against pre-computed table attacks (Rainbow Tables).",
      },
    ],
    git: [
      {
        ...
        answer:
          "Git merge joins two branches together by creating a new merge commit, preserving historical context. Git rebase moves the entire commit history of a branch onto the tip of another branch, producing a clean, linear history.",
      },
    ],
    sql: [
      {
        ...
        answer:
          "Indexes are data structures (typically B-Trees) that speed up database query retrieval. Tradeoff: They slow down write operations (INSERT, UPDATE, DELETE) because the index must be updated, and they consume additional storage disk space.",
      },
    ],
    general: [
      {
        ...
        answer:
          "Clean Code is code that is easy to read, write, and maintain by other developers. It includes sensible naming, single-responsibility functions, extensive testing, and minimal duplication, preventing technical debt.",
      },
    ],
  };
...
export function enrichRoadmapNode(node: RoadmapNode, domainSlug: string): RoadmapNode {
  ...
  if (!enriched.hours || enriched.hours <= 0) {
    enriched.hours =
      enriched.difficulty === "easy" ? 8 : enriched.difficulty === "medium" ? 14 : 20;
  }
    ...
    enriched.projects = [
      {
        ...
        difficulty:
          node.difficulty === "hard" ? "hard" : node.difficulty === "medium" ? "medium" : "easy",
      },
    ];
  }
  ...
  const docSrc =
    key && OFFICIAL_DOCS[key]
      ? OFFICIAL_DOCS[key]
      : OFFICIAL_DOCS[domainSlug]
        ? OFFICIAL_DOCS[domainSlug]
        : null;
  if (docSrc) {
    resources.push({
...
export function generateMindmapData(
  ...
  interviewLeaves.forEach((iItem, idx) => {
    ...
    nodes.push({
      id: leafId,
      label: iItem.length > 25 ? iItem.substring(0, 25) + "..." : iItem,
      type: "leaf",
      x: -280,
      y: 60 + idx * 50,
    });
...
export function getFallbackMindmapAndResources(
  nodeTitle: string,
  domainSlug: string,
  tier: string,
): {
