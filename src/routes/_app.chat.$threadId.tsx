...
import { Send, Loader2, Sparkles, User as UserIcon, Square, ChevronDown, BookOpen, FlaskConical, CalendarClock, Zap } from "lucide-react";
...
const MentorHologram = lazy(() => import("@/components/MentorHologram").then(m => ({ default: m.MentorHologram })));
...
function ChatWindow({
  ...
  const [responseMode, setResponseMode] = useState<"default" | "research" | "learning" | "planning">("default");
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    sendMessage({ id: userMsgId, role: "user", parts: [{ type: "text", text: finalPrompt }] } as any);
    setFiles([]);
  };
  ...
  const handleSaveEdit = async (newText: string) => {
    ...
    try {
      ...
      if (msgIndex !== -1) {
        ...
        const updatedMessage = { ...messages[msgIndex], content: newText, parts: [{ type: "text", text: newText }] };
  ...
  return (
    <div className="flex h-full flex-col relative overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-8 sm:px-8 relative z-10 custom-scrollbar" data-lenis-prevent>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
              ...
              <div className="rounded-2xl border border-border bg-card/60 px-4 py-3">
                <div className="flex gap-1">
                  <span className="h-2 w-2 animate-bounce rounded-full bg-spark" style={{ animationDelay: "0ms" }} />
                  <span className="h-2 w-2 animate-bounce rounded-full bg-spark" style={{ animationDelay: "150ms" }} />
                  <span className="h-2 w-2 animate-bounce rounded-full bg-spark" style={{ animationDelay: "300ms" }} />
                </div>
              </div>
            </motion.div>
          )}
          {error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3 mt-4">
              ...
              <div className="rounded-2xl border border-destructive/50 bg-destructive/10 px-4 py-3 text-sm text-red-400">
                ...
                <p className="mt-1">{error.message || "The AI provider failed to respond. Please check API keys or try again later."}</p>
              </div>
            </motion.div>
          )}
        </div>
      ...
      <form
        onSubmit={onSubmit}
        className="relative z-10 border-t border-white/10 bg-black/40 px-3 pb-3 pt-3 backdrop-blur-3xl sm:px-8"
      ...
      >
        ...
        {/* Main input box */}
        <div className="mx-auto flex max-w-3xl flex-col gap-0 rounded-3xl border border-white/10 bg-white/5 shadow-lg backdrop-blur-2xl transition-all focus-within:border-white/40 focus-within:shadow-[0_0_24px_rgba(255,255,255,0.1)]">
          {/* Textarea row */}
              ...
              <AnimatePresence>
                {modeOpen && (
                  <motion.div
                    ...
                    className="absolute bottom-full left-0 z-50 mb-2 min-w-[160px] overflow-hidden rounded-xl border border-white/10 bg-black/60 shadow-2xl backdrop-blur-2xl"
                  >
                    {([
                      { value: "default",  label: "Default",  icon: Zap,          desc: "Normal answers" },
                      { value: "research", label: "Research", icon: FlaskConical, desc: "Fact-based & cited" },
                      { value: "learning", label: "Learning", icon: BookOpen,     desc: "Structured lessons" },
                      { value: "planning", label: "Planning", icon: CalendarClock,desc: "Step-by-step plans" },
                    ] as const).map(({ value, label, icon: Icon, desc }) => (
                      <button
                        key={value}
                        type="button"
                        onClick={() => { setResponseMode(value); setModeOpen(false); }}
        ...
        <p className="mx-auto mt-2 max-w-3xl text-center text-[10px] text-muted-foreground/50">
          {isStreaming ? "Generating… click stop to cancel." : "ProjectSpark AI · Verify critical information independently."}
        </p>
      </form>
      ...
      {editingMessage && (
        <MessageEditModal
          ...
          onSave={async (newText) => { await handleSaveEdit(newText); }}
        />
      )}
    </div>
  );
...
function Bubble({ m, onEdit }: { m: UIMessage; onEdit?: (m: UIMessage, text: string) => void }) {
  ...
  const { displayContext, files } = isUser ? extractDisplayAndFiles(rawText) : { displayContext: rawText, files: [] };
  ...
  return (
    ...
    >
      <Avatar role={m.role} />
      <div
        className={`max-w-[85%] rounded-3xl border px-5 py-4 text-sm leading-relaxed shadow-xl backdrop-blur-xl ${isUser ? "border-white/20 bg-white/10" : "border-white/5 bg-black/40"}`}
      >
        {isUser ? (
          <div className="flex flex-col gap-2">
            {files.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {files.map(f => (
                  <span key={f} className="text-[10px] flex items-center gap-1 bg-background/50 text-spark px-2 py-0.5 rounded-md border border-spark/20 shadow-sm">
                    📎 {f}
                  </span>
                ))}
              </div>
