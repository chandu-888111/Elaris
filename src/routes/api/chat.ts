      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        try {
          const result = await streamTextResilient({
            ...
            messages: await convertToModelMessages(validMessages as unknown as UIMessage[]),
            abortSignal: request.signal,
          });

          return result.toUIMessageStreamResponse({
            originalMessages: validMessages as unknown as UIMessage[],
          });
        } catch (error) {
          ...
          if (
            ...
            text.includes("secret knowledge")
          ) {
