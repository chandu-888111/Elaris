  ...
  const handleStudySkill = (nodeId: string) => {
    ...
    if (selectedNode?.id === nodeId) {
      setSelectedNode((prev) =>
        prev ? { ...prev, progress: Math.min(100, prev.progress + 10) } : null,
      );
    }
  };
              ...
              {nodes.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(isSelected || isHovered) && <circle r="26" fill="url(#node-glow)" />}

                    {/* Outer Progress Rim */}
                    <circle r="16" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="3.5" />
                    <circle
                      r="16"
                      fill="none"
                      stroke={color}
                    ...
                    {n.progress === 100 && <circle r="4" fill="oklch(0.86 0.1 140)" />}
          ...
          <HolographicPanel className="p-5">
            {selectedNode ? (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-3">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">{selectedNode.info}</p>
                ...
                >
                  ...
                  <span>
                    {selectedNode.progress >= 100 ? "Fully Mastered!" : "Study Concept (+20 XP)"}
                  </span>
                </button>
              </div>
            ) : (
              <div className="text-center py-10">
                ...
                <p className="text-[11px] text-muted-foreground mt-1 px-4 leading-relaxed">
                  Click on any technology node on the visual graph to inspect credentials,
