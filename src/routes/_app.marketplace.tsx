...
const INITIAL_POSTS: ProjectPost[] = [
  {
    ...
    description:
      "Building an automated chest X-ray scanner using vision transformers and React dashboard, offering doctor assistance.",
    author: "arun_singh",
  ...
  },
  {
    ...
    description:
      "Developing cryptography protocols for small embedded devices to assert authenticity without leaking device IDs.",
    author: "lisa_m",
  ...
  },
  {
    ...
    description:
      "Browser extension that audits your cloud resources and estimates real-time carbon footprints of hosting servers.",
    author: "dan_code",
...
export function ProjectMarketplace() {
  ...
  const [matchingRoles, setMatchingRoles] = useState<
    { role: string; matchPct: number; reason: string }[] | null
  >(null);
        ...
        <div className="space-y-6">
          {showAddForm ? (
                ...
                {matchingRoles && (
                  <div className="rounded-xl border border-purple-500/20 bg-purple-500/5 p-3 space-y-2 animate-in slide-in-from-top-2">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-purple-400 flex items-center gap-1">
                      <Icons.Brain className="h-3 w-3 animate-pulse" /> AI Matchmaker
                      Recommendations
                    </div>
            ...
            <HolographicPanel className="p-5 space-y-4">
              <h3 className="font-display text-sm font-bold text-foreground">
                Ecosystem Matchmaking Info
              </h3>
              <p className="text-xs text-muted-foreground leading-relaxed">
                ProjectSpark scans domain, required skills, and study records to pair you with the
                best collaborators. Generate role recommendations instantly with the AI tool.
              </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      Based on current roadmap nodes.
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      Collaborators with high XP are bumped.
                    </div>
                  </div>
                </div>
              </div>
                ...
                <HolographicPanel key={post.id} className="p-5 space-y-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-[10px] text-spark font-medium uppercase tracking-wider mt-0.5">
                        {post.domain}
                      </p>
                    </div>
                  ...
                  {post.suggestedRoles && post.suggestedRoles.length > 0 && (
                    <div className="rounded-xl border border-white/5 bg-white/2 p-3 space-y-2">
                      <div className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-1">
                        <Icons.Brain className="h-3.5 w-3.5 text-spark" /> Suggested Team
                        Matchmaking
                      </div>
