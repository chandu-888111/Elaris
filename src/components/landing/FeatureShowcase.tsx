    ...
    Mockup: () => (
      <div className="w-full h-full rounded-2xl bg-[#0a0a0a] border border-white/10 p-6 flex flex-col gap-4 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden flex flex-col gap-4">
          <div className="self-end glass px-4 py-2 rounded-2xl rounded-tr-sm text-sm text-white max-w-[80%]">Build a Next.js landing page with 3D elements.</div>
          <div className="self-start glass px-4 py-3 rounded-2xl rounded-tl-sm text-sm text-white/80 max-w-[90%] border border-spark/20 bg-spark/5">
            ...
            I'll set up a Next.js project with Three.js and Framer Motion. Here is the architecture...
...
function ShowcaseItem({ showcase, i, total, scrollYProgress }: { showcase: typeof SHOWCASES[0], i: number, total: number, scrollYProgress: MotionValue<number> }) {
  ...
  const zIndex = useTransform(
    scrollYProgress,
    (val) => Math.abs(val - targetScroll) < 0.2 ? 10 : 0
  );

  const pointerEvents = useTransform(
    scrollYProgress,
    (val) => Math.abs(val - targetScroll) < 0.2 ? "auto" : "none"
  );

  return (
    <motion.div 
      style={{ opacity, filter, scale, zIndex, pointerEvents } as any} 
    ...
    <div ref={containerRef} className="relative" style={{ height: `${SHOWCASES.length * 75}vh` }}>
      <div className="sticky top-0 h-screen overflow-hidden">
        {SHOWCASES.map((showcase, i) => (
          <ShowcaseItem key={showcase.id} showcase={showcase} i={i} total={SHOWCASES.length} scrollYProgress={scrollYProgress} />
        ))}
      </div>
    </div>
  );
