...
function OrbitingStations({ count, radius, speed, color }: { count: number, radius: number, speed: number, color: string }) {
  ...
  return (
    <group ref={groupRef}>
      {Array.from({ length: count }).map((_, i) => {
        ...
        return (
          <group key={i} position={[Math.cos(angle) * radius, Math.sin(angle * 2) * 0.2, Math.sin(angle) * radius]}>
            <mesh>
...
function ConstellationLines({ achievements }: { achievements: string[] }) {
  const pts = useMemo(() => {
    ...
    for (let i = 0; i < achievements.length; i++) {
      ...
      const r = 4 + (i % 3);
      points.push(new THREE.Vector3(Math.cos(angle) * r, (Math.random() - 0.5) * 2, Math.sin(angle) * r));
    }
    return points;
  }, [achievements]);
      ...
      {pts.map((p, i) => (
        <Sphere key={i} args={[0.05]} position={p}>
          ...
          <Html distanceFactor={15} center>
            <div className="text-[6px] font-mono text-amber-200/50 uppercase whitespace-nowrap">{achievements[i]}</div>
          </Html>
        </Sphere>
      ))}
      <mesh>
...
export default function UniverseMap({ xp, projects, achievements, skills }: any) {
  ...
  return (
    <>
      ...
      <group position={[0, 0, 0]}>
        ...
        <Html position={[0, -2.5, 0]} center>
          <div className="text-center font-mono">
            <div className="text-white font-bold text-lg whitespace-nowrap uppercase tracking-widest" style={{ color: current.color, textShadow: `0 0 10px ${current.color}` }}>
              {current.name}
            </div>
          </div>
        </Html>
      ...
      <OrbitingStations count={projects?.length || 0} radius={2.5} speed={0.3} color="#38bdf8" />
      <OrbitingStations count={skills?.length || 0} radius={3.5} speed={-0.2} color="#10b981" />
      <ConstellationLines achievements={achievements?.map((a:any) => a.title) || []} />
    </>
  );
}
